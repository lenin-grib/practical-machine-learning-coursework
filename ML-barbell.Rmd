...
According to our test, random forest method did the best in terms of predicting values for the validation dataset with predicted out of sample error of `r 1-as.numeric(as.character(comp[1,2]))`. Boosting came a close second with the OOS error of `r 1-as.numeric(as.character(comp[2,2]))`, but MUCH better performance. The other option would be to limit the predictor to the most important ones (see Appendix 2): even leaving only 5 most significant predictors still gives an estimated OOS error around 0.03 (see Appendix 3).
